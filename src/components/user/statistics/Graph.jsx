...
class Graph extends Component {
  ...
  populateData = (xaxis, array, type) => {
    ...
    this.setState({
      graphData: data,
      currentThermostat: this.props.currentThermostat,
      granularity: type
    });
  };
  ...
  updateGraph = type => {
    ...
    switch (type) {
      ...
      case "Month":
        ...
        this.populateData(
          this.makeMonthAxis(length, this.props.data[0].month),
          data,
          []
        );
        break;
  ...
  makeDayAxis = length => {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      let min = i % 6;
      let axisString = this.padNumber(hour) + ":" + min + "0";
      ...
      <div className="graph-inner-container">
        <ResponsiveContainer>
          <LineChart data={this.state.graphData}>
            ...
            <XAxis
              ticks={this.state.granularity}
              dataKey="name"
              interval={this.state.graphInterval}
            />
            <YAxis
              label={{
